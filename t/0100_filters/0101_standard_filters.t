...
SKIP: {
    ...
    is(
        Template::Liquid->parse('{{date|date:"%Y"}}')
...
SKIP: {
    skip 'Cannot load DateTimeX::Lite module', 1 unless eval { require DateTimeX::Lite; };
    is(
        Template::Liquid->parse(
            '{{date|date:"%Y"}}', date => DateTimeX::Lite->from_epoch( epoch => 0 )
        ),
        1970,
...
is(
    Template::Liquid->parse(q[{{ 'now'|date:"%Y"}}])->render(),
    1900 + [ localtime() ]->[5],
    q[{{ 'now'|date:"%Y"}}]
);
is(
    Template::Liquid->parse(q[{{ 'TODAY'|date:"%Y"}}])->render(),
    1900 + [ localtime() ]->[5],
    q[{{ 'TODAY'|date:"%Y"}}]
);
...
# string/char case
is(
    Template::Liquid->parse(q[{{'this is a QUICK test.'|capitalize}}])->render(),
    'This is a quick test.',
    q[{{'this is a QUICK test.'|capitalize}} => This is a quick test.]
);
is(
    Template::Liquid->parse(q[{{'This is a QUICK test.'|downcase }}])->render(),
    'this is a quick test.',
    q[{{'This is a QUICK test.'|downcase }} => this is a quick test.]
);
is(
    Template::Liquid->parse(q[{{'This is a QUICK test.'|upcase }}])->render(),
    'THIS IS A QUICK TEST.',
...
is(
    Template::Liquid->parse(q[{{array | join:', '}}])->render( array => [ 1 .. 6 ] ),
    '1, 2, 3, 4, 5, 6',
    q[{{array | join:', ' }} => 1, 2, 3, 4, 5, 6]
);
note 'For this next test, C<array> is defined as C<[10,62,14,257,65,32]>';
is( Template::Liquid->parse(q[{{array | sort}}])->render( array => [ 10, 62, 14, 257, 65, 32 ] ),
...
is( Template::Liquid->parse(q[{{array | size}}])->render( array => [ 0 .. 10 ] ),
    '11', '{{array | size}} => 11' );
is(
    Template::Liquid->parse(q[{{hash | size}}])
        ->render( hash => { Beatles => 'Apple', Nirvana => 'SubPop' } ),
    '2',
    q[{{hash | size}} => 2 (counts keys)]
);
is( Template::Liquid->parse(q[{{nope | size}}])->render(), '0', q[{{nope | size}} => 0 (undef)] );

# split
is(
    Template::Liquid->parse(q[{{ values | split: ',' | last }}])->render( values => 'foo,bar,baz' ),
    'baz',
    q[{{ values | split: ',' | last}}]
);
...
is(
    Template::Liquid->parse(q[{{ '<div>Hello, <em id="whom">world!</em></div>' | strip_html}}])
        ->render(),
    'Hello, world!',
    q[{{'<div>Hello, <em id="whom">world!</em></div>'|strip_html}} => Hello, world!]
);
is(
    Template::Liquid->parse(q['{{ '<IMG SRC = "foo.gif" ALT = "A > B">' | strip_html}}'])->render(),
    q[' B">'],
    q['{{ '<IMG SRC = "foo.gif" ALT = "A > B">'|strip_html }}' => ' B">']
);
is(
    Template::Liquid->parse(q['{{ '<!-- <A comment> -->' | strip_html }}'])->render(),
    q[' -->'],
...
is(
    Template::Liquid->parse('{{multiline|strip_newlines}}')
        ->render( multiline => qq[This\n is\n a\n test.] ),
    'This is a test.',
    q[{{multiline|strip_newlines}} => 'This is a test.']
);
is(
    Template::Liquid->parse('{{multiline|newline_to_br}}')
        ->render( multiline => qq[This\n is\n a\n test.] ),
    qq[This<br />\n is<br />\n a<br />\n test.],
...
is(
    Template::Liquid->parse(q[{{'Replace that with this'|replace:this,'this'}}])
        ->render( this => 'that' ),
    'Replace this with this',
    q[{{'Replace that with this|replace:this,'this'}} => Replace this with this]
);
is(
    Template::Liquid->parse(q[{{'I have a listhp.'|replace:'th'}}])->render(),
    'I have a lisp.',
    q[{{'I have a listhp.'|replace:'th'}} => I have a lisp.]
);
is( Template::Liquid->parse(q[{{ 'barbar' | replace_first:'bar','foo' }}])->render(),
...
# truncation
is(
    Template::Liquid->parse(q[{{ 'Running the halls!!!' | truncate:19 }}])->render(),
    'Running the hall...',
    q[{{ 'Running the halls!!!' | truncate:19 }} => Running the hall...]
);
note q[This next method uses C<blah> which is defined as C<'STOP!'>];
is(
    Template::Liquid->parse(q[{{ 'Any Colour You Like' | truncate:10,blah }}])
        ->render( blah => 'STOP!' ),
    'Any CSTOP!',
...
is( Template::Liquid->parse(q[{{ 'bar' | prepend:baz }}])->render( baz => 'foo' ),
    'foobar', q[ {{ 'bar' | prepend:baz }} => 'foobar'] );
is( Template::Liquid->parse(q[{{ 'bar' | prepend:baz }}])->render( baz => '' ),
    'bar', q[ {{ 'bar' | prepend:baz }} => 'bar'] );
is( Template::Liquid->parse(q[{{ 'bar' | prepend:baz }}])->render( xxx => '' ),
    'bar', q[ {{ 'bar' | prepend:baz }} => 'bar'] );
is( Template::Liquid->parse(q[{{ 'bar' | prepend:'baz' }}])->render( baz => 'fun' ),
    'bazbar', q[ {{ 'bar' | prepend:'baz' }} => 'bazbar'] );
...
# subtraction
is( Template::Liquid->parse(q[{{ 4|minus:2 }}])->render(),      '2', q[{{ 4|minus:2 }} => 2] );
is( Template::Liquid->parse(q[{{ 'Test'|minus:2 }}])->render(), '',  q[{{ 'Test'|minus:2 }} => ] );
is( Template::Liquid->parse(q[{{ 4.3|minus:2.5 }}])->render(),
    '1.8', q[{{ 4.3|minus:2.5 }} => 1.8] );
is( Template::Liquid->parse(q[{{ -4|minus:2 }}])->render(), '-6', q[{{ -4|minus:2 }} => -6] );
...
is( Template::Liquid->parse(q[{{ 15| plus:-11 }}])->render(), '4', q[{{ 15| plus:-11 }} => 4] );
...
is( Template::Liquid->parse(q[{{ 5|times:4 }}])->render(), '20', q[{{ 5|times:4 }} => 20] );
...
# modulo
is( Template::Liquid->parse(q[{{ 95 | modulo:6 }}])->render(), '5', q[{{ 95 | modulo:6 }} => 5] );
...
is(
    Template::Liquid->parse(q[{%for row in rows%}{{row.a | minus: row.b}} {%endfor%}])->render(
        rows => [
            { a => 5,   b => 2 },
            { a => 13,  b => 12 },
            { a => 13,  b => 112 },
...
# round
is(
    Template::Liquid->parse(q[{{ 4.6 | round }} {{ 4.3 | round }} {{ 4.5612 | round: 2 }}])
        ->render(),
    '5 4 4.56',
