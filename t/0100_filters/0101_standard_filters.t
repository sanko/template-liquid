...
SKIP: {
    ...
    is( Solution::Template->parse('{{date|date:"%Y"}}')
...
is(Solution::Template->parse('{{date|date:"%Y"}}')->render({date => gmtime(0)}),
    1970, '{{date|date:"%Y"}} => 1970 (int)');

# string/char case
is( Solution::Template->parse(q[{{'this is a QUICK test.'|capitalize}}])->render(),
    'This is a quick test.',
    q[{{'this is a QUICK test.'|capitalize}} => This is a quick test.]
);
is( Solution::Template->parse(q[{{'This is a QUICK test.'|downcase }}])->render(),
    'this is a quick test.',
    q[{{'This is a QUICK test.'|downcase }} => this is a quick test.]
);
is( Solution::Template->parse(q[{{'This is a QUICK test.'|upcase }}])->render(),
    'THIS IS A QUICK TEST.',
...
is(Solution::Template->parse(q[{{array | first}}])->render({array => [1 .. 6]}),
    '1', '{{array | first}} => 1');
is(Solution::Template->parse(q[{{array | last}}])->render({array => [1 .. 6]}),
    '6', '{{array | last }} => 6');
is(Solution::Template->parse(q[{{array | join}}])->render({array => [1 .. 6]}),
...
is( Solution::Template->parse(q[{{array | sort}}])
...
is(Solution::Template->parse(q[{{'This is a test' | size}}])->render(),
    '14', q[{{'This is a test' | size}} => 14]);
is(Solution::Template->parse(q[{{array | size}}])->render({array => [0 .. 10]}),
    '11', '{{array | size}} => 11');
is( Solution::Template->parse(q[{{hash | size}}])
...
is( Solution::Template->parse(q['{{ '<!-- <A comment> -->' | strip_html }}'])
        ->render(),
    q[' -->'],
...
is( Solution::Template->parse('{{multiline|strip_newlines}}')
        ->render({multiline => qq[This\n is\n a\n test.]}),
    'This is a test.',
    q[{{multiline|strip_newlines}} => 'This is a test.']
);
is( Solution::Template->parse('{{multiline|newline_to_br}}')
        ->render({multiline => qq[This\n is\n a\n test.]}),
    qq[This<br />\n is<br />\n a<br />\n test.],
...
is(Solution::Template->parse(q[{{'foofoo'|replace:'foo', 'bar'}}])->render(),
    'barbar', q[{{'foofoo'|replace:'foo', 'bar'}} => barbar]);
note(q[This next method uses C<this> which is defined as C<'that'>]);
is( Solution::Template->parse(q[{{'Replace that with this'|replace:this,'this'}}])
        ->render({this => 'that'}),
    'Replace this with this',
    q[{{'Replace that with this|replace:this,'this'}} => Replace this with this]
);
is(Solution::Template->parse(q[{{'I have a listhp.'|replace:'th'}}])->render(),
    'I have a lisp.',
    q[{{'I have a listhp.'|replace:'th'}} => I have a lisp.]);
is( Solution::Template->parse(q[{{ 'barbar' | replace_first:'bar','foo' }}])->render(),
    'foobar', q[{{ 'barbar' | replace_first:'bar','foo' }} => foobar]
);
is(Solution::Template->parse(q[{{ 'foobarfoobar' | remove:'foo' }}])->render(),
    'barbar', q[{{ 'foobarfoobar' | remove:'foo' }} => barbar]);
is(Solution::Template->parse(q[{{ 'barbar' | remove_first:'bar' }}])->render(),
    'bar', q[{{ 'barbar' | remove_first:'bar' }} => bar]);

# truncation
is( Solution::Template->parse(q[{{ 'Running the halls!!!' | truncate:19 }}])->render(),
    'Running the hall...',
    q[{{ 'Running the halls!!!' | truncate:19 }} => Running the hall...]
);
note(q[This next method uses C<blah> which is defined as C<'STOP!'>]);
is( Solution::Template->parse(q[{{ 'Any Colour You Like' | truncate:10,blah }}])
        ->render({blah => 'STOP!'}),
    'Any CSTOP!',
    q[{{ 'Any Colour You Like' | truncate:10,blah }} => Any CSTOP!]
);
is(Solution::Template->parse(q[{{ 'bar' | prepend:'foo' }}])->render(),
...
is(Solution::Template->parse(q[{{ 'bar' | prepend:'foo' }}])->render(),
    'foobar', q[{{ 'bar' | prepend:'foo' }} => 'foobar']);
is(Solution::Template->parse(q[{{ 'foo' | append:'bar' }}])->render(),
    'foobar', q[{{ 'foo' | append:'bar' }} => 'foobar']);

# subtraction
is(Solution::Template->parse(q[{{ 4|minus:2 }}])->render, '2',
    q[{{ 4|minus:2 }} => 2]);
is(Solution::Template->parse(q[{{ 'Test'|minus:2 }}])->render,
    '', q[{{ 'Test'|minus:2 }} => ]);

# concatenation or simple addition
is(Solution::Template->parse(q[{{ 154| plus:1183 }}])->render(),
    '1337', q[{{ 154| plus:1183 }} => 1337]);
is(Solution::Template->parse(q[{{ 'W'| plus:'TF' }}])->render(),
    'WTF', q[{{ 'W'| plus:'TF' }} => WTF]);

# multiplication or string repetion
is(Solution::Template->parse(q[{{ 'foo'| times:4 }}])->render(),
    'foofoofoofoo', q[{{ 'foo'|times:4 }} => foofoofoofoo]);
is(Solution::Template->parse(q[{{ 5|times:4 }}])->render(),
    '20', q[{{ 5|times:4 }} => 20]);

# division
is(Solution::Template->parse(q[{{ 10 | divided_by:2 }}])->render(),
